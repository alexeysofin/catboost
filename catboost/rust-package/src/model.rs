...
impl Model {
    ...
    >(
        ...
        let mut text_features_ptr = text_features_ptr_storage
            .iter_mut()
            .map(|object_texts_ptrs: &mut Vec<*const std::os::raw::c_char>| object_texts_ptrs.as_mut_ptr())
            .collect::<Vec<_>>();
    ...
    /// Calculate single raw model prediction on float features and string categorical feature values
    ...
    >(
        ...
        let mut cat_features_ptrs = cat_features
            .as_ref()
            .iter()
            .map(|f| f.as_ref().as_ptr() as *const std::os::raw::c_char)
            .collect::<Vec<_>>();
    ...
    pub fn get_model_used_features_names(&self) -> CatBoostResult<Vec<String>> {
        let mut names_raw: *mut *mut std::os::raw::c_char = null_mut();
